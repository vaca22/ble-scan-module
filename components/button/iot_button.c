...
#define CALL_EVENT_CB(ev) \
    if (btn->cb[ev]) btn->cb[ev](btn->usr_data);
...
static button_dev_t *button_create_com(
    uint8_t active_level, uint8_t (*hal_get_key_state)(void *usr_data),
    void *usr_data) {
...
static esp_err_t button_delete_com(button_dev_t *btn) {
    ...
    if (0 == number &&
        g_is_timer_running) { /**<  if all button is deleted, stop the timer */
...
button_handle_t iot_button_create(const button_config_t *config) {
    ...
    switch (config->type) {
        case BUTTON_TYPE_GPIO: {
            ...
            btn =
                button_create_com(cfg->active_level, button_gpio_get_key_level,
                                  (void *)cfg->gpio_num);
        } break;
        case BUTTON_TYPE_ADC: {
            ...
            btn = button_create_com(1, button_adc_get_key_level,
                                    (void *)ADC_BUTTON_COMBINE(
                                        cfg->adc_channel, cfg->button_index));
        } break;
...
esp_err_t iot_button_delete(button_handle_t btn_handle) {
    esp_err_t ret = ESP_OK;
    BTN_CHECK(NULL != btn_handle, "Pointer of handle is invalid",
              ESP_ERR_INVALID_ARG);
    ...
    switch (btn->type) {
        case BUTTON_TYPE_GPIO:
            ...
            break;
        case BUTTON_TYPE_ADC:
            ret = button_adc_deinit(ADC_BUTTON_SPLIT_CHANNEL(btn->usr_data),
                                    ADC_BUTTON_SPLIT_INDEX(btn->usr_data));
            break;
        default:
            break;
    }
...
esp_err_t iot_button_register_cb(button_handle_t btn_handle,
                                 button_event_t event, button_cb_t cb) {
    BTN_CHECK(NULL != btn_handle, "Pointer of handle is invalid",
              ESP_ERR_INVALID_ARG);
    BTN_CHECK(event < BUTTON_EVENT_MAX, "event is invalid",
              ESP_ERR_INVALID_ARG);
    button_dev_t *btn = (button_dev_t *)btn_handle;
    btn->cb[event]    = cb;
    return ESP_OK;
