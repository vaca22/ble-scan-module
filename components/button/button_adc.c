...
esp_err_t button_adc_init(const button_adc_config_t *config) {
    ADC_BTN_CHECK(NULL != config, "Pointer of config is invalid",
                  ESP_ERR_INVALID_ARG);
    ADC_BTN_CHECK(config->adc_channel < ADC1_CHANNEL_MAX,
                  "channel out of range", ESP_ERR_NOT_SUPPORTED);
    ADC_BTN_CHECK(config->button_index < ADC_BUTTON_MAX_BUTTON,
                  "button_index out of range", ESP_ERR_NOT_SUPPORTED);
    ADC_BTN_CHECK(config->max > 0, "key max voltage invalid",
                  ESP_ERR_INVALID_ARG);
    ...
    if (0 == g_button.is_configured) {
        ...
        esp_adc_cal_value_t val_type = esp_adc_cal_characterize(
            ADC_BUTTON_ADC_UNIT, ADC_BUTTON_ATTEN, ADC_BUTTON_WIDTH,
            DEFAULT_VREF, &g_button.adc_chars);
...
esp_err_t button_adc_deinit(adc1_channel_t channel, int button_index) {
    ADC_BTN_CHECK(channel < ADC1_CHANNEL_MAX, "channel out of range",
                  ESP_ERR_INVALID_ARG);
    ADC_BTN_CHECK(button_index < ADC_BUTTON_MAX_BUTTON,
                  "button_index out of range", ESP_ERR_INVALID_ARG);
    ...
    if (unused_button == ADC_BUTTON_MAX_BUTTON &&
        g_button.ch[ch_index]
            .is_init) { /**< if all button is unused, deinit the channel */
        /* TODO: to deinit the channel  */
        g_button.ch[ch_index].is_init = 0;
        g_button.ch[ch_index].channel = ADC1_CHANNEL_MAX;
        ESP_LOGD(TAG, "all button is unused on channel%d, deinit the channel",
                 g_button.ch[ch_index].channel);
    }
    ...
    if (unused_ch == ADC_BUTTON_MAX_CHANNEL &&
        g_button
            .is_configured) { /**< if all channel is unused, deinit the adc */
...
static uint32_t get_adc_volatge(adc1_channel_t channel) {
    ...
    uint32_t voltage =
        esp_adc_cal_raw_to_voltage(adc_reading, &g_button.adc_chars);
...
uint8_t button_adc_get_key_level(void *button_index) {
    ...
    ADC_BTN_CHECK(index < ADC_BUTTON_MAX_BUTTON, "button_index out of range",
                  0);
